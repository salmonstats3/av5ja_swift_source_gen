...
export class SHA256Hash {
    ...
    static async write(): Promise<void> {
        ...
        const source: string[] = [
            ...
            '',
          ].concat(hashes.sort((a, b) => a.id > b.id ? 1 : -1).map((hash) => `\t case ${hash.id} = "${hash.key}"`))
          source.push('}');
    ...
    private static async get_hash(): Promise<Hash[]> {
        ...
        const url: string = `https://api.lp1.av5ja.srv.nintendo.net/static/js/main.${hash}.js`
        ...
        const hashes = [...matches].map((match) => new Hash(match[2], match[1]))
