...
async function get_locales(hash: string): Promise<LocaleType[]> {
  ...
  const results = [...response.matchAll(hashes)]
    ...
    .map((item) =>
      plainToInstance(LocaleType, item, { excludeExtraneousValues: true }),
    )
    .concat([
      plainToInstance(
        LocaleType,
...
export class LocaleType {
  ...
  async get_translation(): Promise<Translation> {
    const locale: string =
      Object.keys(LocaleId)[Object.values(LocaleId).indexOf(this.id)];
    ...
    return plainToInstance(Translation, objects, {
      excludeExtraneousValues: true,
    });
  }
  ...
  @Transform(
    (param) =>
      Object.values(LocaleId)[
        (Object.values(LocaleId) as number[]).indexOf(
          Number.parseInt(param.value, 10),
        )
      ],
  )
   ...
   */
  get key(): LocaleKey {
    return Object.values(LocaleKey)[
      (Object.values(LocaleId) as number[]).indexOf(this.id)
    ];
  }
