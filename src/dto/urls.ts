...
export async function request(
  ...
  switch (url) {
    case URLType.WeaponInfoMain:
      return [
        WeaponInfoMain.Dummy,
        WeaponInfoMain.Random_Gold,
        WeaponInfoMain.Random_Green,
      ]
        .concat(
          response
            .map((item: any) =>
    ...
    case URLType.WeaponInfoSpecial:
      return [WeaponInfoSpecial.Random_Green]
        .concat(
          response
            .map((item: any) =>
              plainToInstance(WeaponInfoSpecial, item, {
                excludeExtraneousValues: true,
              }),
            )
    ...
    case URLType.GearInfoHead:
      return response
        .map((item: any) =>
          plainToInstance(GearInfo, item, { excludeExtraneousValues: true }),
        )
        .sort((a, b) => a.id - b.id);
    ...
    case URLType.CoopSkinInfo:
      return response
        .map((item: any) =>
          plainToInstance(CoopSkinInfo, item, {
            excludeExtraneousValues: true,
          }),
        )
        .sort((a, b) => a.id - b.id);
    ...
    case URLType.BadgeInfo:
      return response
        .map((item: any) =>
          plainToInstance(BadgeInfo, item, { excludeExtraneousValues: true }),
        )
        .sort((a, b) => a.id - b.id);
